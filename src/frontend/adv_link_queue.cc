...
AdvLinkQueue::AdvLinkQueue( const string & link_name, const string & filename, const string & logfile,
                      const bool repeat, const bool graph_throughput, const bool graph_delay,
                      unique_ptr<AbstractPacketQueue> && packet_queue,
                      const string & command_line, const string & adversary_logfile )
    : next_delivery_( 0 ),
      schedule_(),
      ...
      finished_( false )
{
    ...
    /* create adversary, if logfile is defined */
    if ( not adversary_logfile.empty() ) {
        adversary_.reset( new Adversary( adversary_logfile ));
    }
}

void AdvLinkQueue::record_arrival( const uint64_t arrival_time, const size_t pkt_size )
{
    /* log it */
    if ( log_ ) {
...
void AdvLinkQueue::record_drop( const uint64_t time, const size_t pkts_dropped, const size_t bytes_dropped)
{
    /* log it */
    if ( log_ ) {
...
void AdvLinkQueue::record_departure_opportunity( void )
{
    ...
    if ( adversary_ ) {
        /* record packet departure opportunity */
        // TOTEST
        adversary_->record_packet_departure_opportunity();
    }

}

void AdvLinkQueue::record_departure( const uint64_t departure_time, const QueuedPacket & packet )
{
    /* log the delivery */
    if ( log_ ) {
    ...
    if ( adversary_ )
    {
        /* record packet departure in adversary */
        // TOTEST
        adversary_->record_packet_departure();
        /* record queuing delay in adversary */
        // TOTEST
        adversary_->record_queuing_delay( departure_time - packet.arrival_time );
    }
}

void AdvLinkQueue::read_packet( const string & contents )
{
...
uint64_t AdvLinkQueue::next_delivery_time( void ) const
{
    if ( finished_ ) {
        return -1;
    } else {
...
void AdvLinkQueue::use_a_delivery_opportunity( void )
{
...
void AdvLinkQueue::rationalize( const uint64_t now )
{
...
void AdvLinkQueue::write_packets( FileDescriptor & fd )
{
...
unsigned int AdvLinkQueue::wait_time( void )
{
...
bool AdvLinkQueue::pending_output( void ) const
{
    return not output_queue_.empty();
}

void AdvLinkQueue::update_schedule( std::vector<uint64_t> new_schedule )
{
...
Adversary::Adversary(const std::string &adversary_logfile)
    : num_packet_departure_opportunity_(),
      num_packet_departure_(),
      current_queuing_delay_(),
      log_(),
      halt_( false ),
      probe_thread_exception_(),
      probe_thread_( [&] () {
              try {
                  probe_loop();
              } catch ( ... ) {
      ...
      //adv_link_queue_()
{
    /* open logfile if called for */
    if ( not adversary_logfile.empty() ) {
        log_.reset( new std::ofstream( adversary_logfile ) );
        if ( not log_->good() ) {
            throw std::runtime_error( adversary_logfile + ": error opening for writing" );
        }
        *log_ << "time, link_utilization, queueing_delay" << std::endl;
    }
    probe_time_ = PROBE_PER;
}
...
Adversary::~Adversary()
{
    ...
    probe_thread_.join();
     ...
     if ( probe_thread_exception_ != exception_ptr() ) {
        try {
            rethrow_exception( probe_thread_exception_ );
        } catch ( const exception & e ) {
            cerr << "Adversary exited from exception: ";
            print_exception( e );
        }
    }
}

void Adversary::probe_loop(void)
{
    while ( not halt_ ) {
        const uint64_t now = timestamp();
        float link_util = get_link_utilization();
        uint64_t queueing_delay = get_queueing_delay();
        log_state(now, link_util, queueing_delay);
        std::this_thread::sleep_for(std::chrono::milliseconds(probe_time_));
    }
}


void Adversary::record_packet_departure_opportunity( void )
{
    num_packet_departure_opportunity_ += 1;
}

void Adversary::record_packet_departure( void )
{
    num_packet_departure_ += 1;
}

double Adversary::get_link_utilization( void )
{
...
void Adversary::record_queuing_delay( uint64_t queuing_delay )
{
    current_queuing_delay_ = queuing_delay;
}

uint64_t Adversary::get_queueing_delay( void )
{
    return current_queuing_delay_; 
}

void Adversary::log_state( const uint64_t now, double link_util, uint64_t queueing_delay )
{
    if ( log_ ) 
    {
...
void Adversary::start_adversary_loop( void )
{
    // main adversary loop
}
